  ...
  const fetchProduct = async () => {
    try {
      ...
      if (res.ok) {
        ...
        console.error("Failed to load product:", res.status, await res.text());
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-4">
          <div className="flex flex-col sm:flex-row gap-4">
            <button
              type="submit"
              disabled={saving}
              className="px-10 py-4 bg-sky-500 text-white rounded-full font-semibold hover:bg-sky-600 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2"
            >
              {saving ? (
                <>
            ...
            <Link
              href="/admin/products"
              className="px-10 py-4 border-2 border-black/10 rounded-full font-semibold hover:border-black/30 hover:bg-gray-50 transition-all flex items-center justify-center gap-2"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            disabled={deleting}
            className="px-8 py-4 bg-red-50 text-red-600 border-2 border-red-200 rounded-full font-semibold hover:bg-red-100 hover:border-red-300 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
          >
            {deleting ? (
              <>
                <svg className="animate-spin h-5 w-5" fill="none" viewBox="0 0 24 24">
